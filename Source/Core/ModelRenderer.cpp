...
void Candela::RenderEntity(Entity& entity, GLClasses::Shader& shader, Frustum& frustum, bool fcull, int entity_num, bool transparent_pass)
{
	...
	for (auto& e : object->m_Meshes)
	{
		...
		shader.SetVector3f("u_ModelColor", (entity.m_OverrideColor.x >= 0.0f && entity.m_OverrideColor.y >= 0.0f && entity.m_OverrideColor.z >= 0.0f) ? entity.m_OverrideColor : mesh->m_Color);
